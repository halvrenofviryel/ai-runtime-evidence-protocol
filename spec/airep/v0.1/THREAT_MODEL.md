...
- **A signed chain head / length witness** (or a transparency-log anchor) closes **tail truncation**
  ...
  **now shipped** as the [`chain_witness`](./profiles/chain_witness.schema.json) profile, with a worked
  ...
  truncation-detection check in `validate.py`. It is the profile AIREP-Trusted requires — but
  carrying it does **not** currently lift a record to that tier: the classifiers check witness
  *presence* only, leaving four Trusted prerequisites unenforced, so they withhold the top class and
  report `TRUSTED_NOT_IMPLEMENTED` (see
