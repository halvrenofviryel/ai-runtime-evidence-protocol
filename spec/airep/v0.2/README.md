...
| [`schemas/`](./schemas/) | The five JSON Schema (2020-12) files mechanically expressing the accepted design contract: `common` (`$defs` only — not a standalone validator) + `decision`/`control`/`execution`/`effect` entry schemas owning their top-level closure. Schema validation confers no assurance class, signature validity, or evidence truth. |
