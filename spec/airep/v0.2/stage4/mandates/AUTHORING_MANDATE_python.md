# Authoring mandate — Stage-4 Python integrity verifier (process record)

> Recorded verbatim from the authoring instruction given to the isolated agent that wrote
> `verifier_py/integrity_verifier.py`. This is a process-constraint record for the
...
- `spec/airep/v0.1/conformance/jcs.py` (pre-existing RFC 8785 implementation; MAY be reused)
...
- the Node Stage-4 verifier (`spec/airep/v0.2/stage4/verifier_node/`) or its output/results
...
`witnessed_at` vs fixture `now`/window with full §4.2 Gregorian validation; head-derived
witness tag version; head reconciliation; no fallback/search anywhere; deterministic sorted
results file with no metadata; exit 0 iff every fixture received a result; two runs
byte-identical.
