...
| `WITNESS_CLAIM_INVALID` | REJECT | The signed head claim violates the closed five-member structure of INTEGRITY §4 — the member set is not exactly `{chain_id, sequence, current, length, witnessed_at}`, or a pinned non-time member constraint fails (`chain_id` not a string; `sequence` not a non-negative safe integer; `current` not exactly `sha256:` + 64 lowercase hex; `length` not a positive safe integer). Time-semantics violations of `witnessed_at` are `WITNESS_TIME_INVALID`, not this code. |
