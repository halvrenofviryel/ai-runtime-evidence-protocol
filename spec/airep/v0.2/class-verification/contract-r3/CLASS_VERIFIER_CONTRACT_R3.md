# AIREP v0.2 — Class-Verifier Implementation Contract, revision **r3**

> **Effective contract, stated mechanically — no inference required:**
>
> ```
> effective r3 contract = this document
>                       + r1 (sha256:7ecfce56ab576a495816df77e25442b25c1afdb22cc9828e47ba29a565138885)
>                         restricted to the surfaces R3.1 incorporates by reference
> ```
>
> **r2 is not an input.** This document does not depend on r2 in any way; r2 is retained only as
> superseded historical text. There is no `r2 + patch` composition to reconstruct.
>
> **r1 IS a load-bearing input**, incorporated by explicit reference for the surfaces enumerated in
> R3.1 and for nothing else. Composition order: **this document governs**; where any r3 clause and an
> incorporated r1 clause conflict, **r3 prevails**. r1 is frozen and never edited, so the
> incorporation is stable and reproducible from its digest.
>
> An implementer therefore needs exactly three artefacts: this document, r1, and
> [`SOURCE_BASIS.json`](./SOURCE_BASIS.json), plus the normative AIREP basis they name.
...
(`airep_version` is pinned `"const": "0.2"` in the shared schema). `r1` / `r2` / `r3` identify
*measurement-contract revisions* only. An artifact never carries a contract revision.

---

## R3.0 Semantic basis identities
...
| Predecessor measurement contract (**r2**, superseded before use) | `spec/airep/v0.2/class-verification/contract-r2/CLASS_VERIFIER_CONTRACT_R2.md` `sha256:630676b1a199a5453e96b5e18890b17a4d0e5c290c2156aa15f1a3f899aad8a0`, commit `1400aa2451d0b94d07a5672a0baf404bac8448c8` |
| Original measurement contract (**r1**, historical) | `spec/airep/v0.2/class-verification/CLASS_VERIFIER_CONTRACT.md` `sha256:7ecfce56ab576a495816df77e25442b25c1afdb22cc9828e47ba29a565138885` |
...
```text
r1  sha256:7ecfce56...   historical - basis of every measurement recorded to date
  |
  | profile evaluation not represented
  v
r2  sha256:630676b1...   SUPERSEDED BEFORE IMPLEMENTATION/MEASUREMENT
  |                      validation-basis closure was undefined (R2-D1, R2-D2 below)
  |
  v
r3                       current, standalone
  |
  v
new corpus / parity evidence
```

**No implementation, corpus, parity result or external result exists against r2.** It was superseded
before any implementation was authored against it, on a read-only audit — not because a measurement
failed. It is neither failed external evidence nor a measured contract.

**Neither r2 nor r3 is the basis of any historical measurement.** The C1 parity run, the historical
60-case corpus, and every external run recorded to date were measured against **r1** and remain so.
No historical external implementation is claimed to implement or pass r2 or r3.

### Why r3 exists — two validation-basis closure defects in r2

A read-only closure audit, before any implementation, asked: *does
`profile_evaluations.<id>.basis_digest` identify the complete, unambiguous validation semantics used
for that profile?* Under r2 the answer was **no**, twice.

**R2-D1 — duplicate member names inside the validation basis itself.** r2 required the *registry* to
reject duplicate JSON member names, and stated the reason correctly, but scoped the rule to the
registry object only. A basis such as `{"type":"object","required":["a"],"required":["b"]}` has
parser-dependent semantics under last-key-wins parsing while `basis_digest` stays identical — the
digest would identify bytes, not semantics.

**R2-D2 — transitive `$ref` dependency closure undefined.** r2 defined `basis_digest` as the SHA-256
of the root document only, and never mentioned `$ref`, self-containment or transitive resources. Two
runs could report the same `basis_digest` while resolving different referenced bytes.

Both are corrected in R3.4 below. Nothing else in r2's semantics is changed.

---

## R3.1 Surfaces incorporated from r1 by normative reference (r1 is load-bearing)

The following r1 surfaces are **normatively incorporated by reference**. r1 is therefore a
**load-bearing input** to the effective contract, not merely provenance. Everything in r1 not
restated or superseded by this document is carried **unchanged**: the evaluation-request envelope (r1 §0); operator-input formats for bindings, independence
...
r1 is **not edited** and its frozen copies in the interop handoff packages are **not replaced**. r2 is
likewise not edited; it is retained byte-exact as superseded text.

---

## R3.2 Additive verdict channel: `profile_evaluations`
...
## R3.3 Profile-validation basis input (operator input, network-free)
...
registry containing a duplicate member name **MUST** be rejected as unusable (R3.5), specifically:
...
requirement is scoped to the profile-basis registry defined by this contract (R3.3) only.** It does not alter historical artifact
...
Reject, as an unusable basis (R3.5): absolute paths; `..` traversal leaving the root; symlink-mediated
...
## R3.4 Validation-basis closure — the basis bytes name the complete semantics

`basis_digest` MUST identify **all bytes that can affect profile-validation semantics**. Three rules
close that, and each is testable.

### R3.4.1 Mandatory processing order

Duplicate-member information is destroyed by an ordinary last-key-wins parser, so it MUST be detected
during parsing, never on an already-parsed object. The required order is:

```text
raw basis bytes
  -> UTF-8 / JSON parse WITH duplicate-member detection
  -> self-contained-resource check
  -> dialect / configuration check
  -> semantic schema compilation
  -> payload evaluation
```

**No object produced by a last-key-wins parser may become the input to duplicate checking**, because
by then the duplicate information is already lost. The same ordering requirement applies to the
profile-basis registry duplicate-key rule in R3.3.

### R3.4.2 Unambiguous basis interpretation

> **JSON-based validation-basis documents MUST reject duplicate object member names at every depth,
> before semantic validation.**

A basis with a duplicate member name at any depth is **unusable** (R3.5) — never `PASS`, never
`FAIL`, because the profile was never validly evaluated.

### R3.4.3 Model 1 — the basis MUST be self-contained

> **A validation-basis document MUST be self-contained. Any dependency on bytes outside the exact
> basis document is prohibited.**

| Allowed | Forbidden |
|---|---|
| no `$ref` at all | file references (`file:`, or any relative/absolute filesystem path) |
| fragment-only references whose complete target is inside the same document (e.g. `#/$defs/x`), valid under the pinned dialect | HTTP(S) or any network reference |
| | relative external resources |
| | absolute external resources |
| | any reference requiring bytes outside the exact basis document |
| | any implicit external resolver, plugin or registry lookup |

**"No network" is not sufficient.** A *local* external file is still an undeclared transitive
dependency and is equally forbidden. A network reference MUST be rejected **without attempting
retrieval**.

A basis violating this is **unusable** (R3.5), never `FAIL`.

**Consequence:** `basis_digest = sha256(exact self-contained basis document bytes)` identifies the
complete validation semantics for that identifier.

**Model 2 (explicit basis bundle) is deliberately NOT adopted.** Permitting dependencies would
require binding the complete transitive resource set — every resource identity, every exact byte
digest, deterministic resolution, no network, no undeclared dependency, and a reproducible bundle
identity. r3 does not define that machinery and no implementation may assume it.

### R3.4.4 One pinned dialect and configuration

The same bytes MUST NOT yield different semantics because a host validator assumed a different
dialect. The interpretation is **pinned by this contract, never selected by the engine**, and follows
the discipline already adopted for the Core schema-validation phase.

- **Dialect: JSON Schema Draft 2020-12.**
  - `$schema` **absent** ⇒ the document is still interpreted as Draft 2020-12.
  - `$schema` **present and identifying the Draft 2020-12 meta-schema** ⇒ acceptable.
  - `$schema` **present and declaring any other dialect** ⇒ **basis unusable** for this evaluator.
  - An engine MUST NOT auto-select a dialect from `$schema`.
- **`format` is an annotation and is NOT assertion-bearing.** No format validation is performed.
  This matches the accepted Core discipline, which *"gives `format` no semantic weight"*. The two
  implementations MUST NOT diverge because one has optional format validators installed.
- **Vocabularies.** No custom or external semantic vocabulary may silently alter evaluation. A basis
  requiring `$vocabulary` support beyond the pinned Draft 2020-12 dialect is **unusable**. Plugins
  and network vocabularies are never loaded implicitly. This does **not** ban harmless unknown
  annotation keywords whose treatment Draft 2020-12 already determines.
- **Engine identity remains run provenance.** The dialect and configuration are defined here; an
  engine merely implements them, so different engines may be measured against the same basis.

### R3.4.5 Required closure probes

Beyond the negative proofs in R3.8, these MUST be exercised:

1. no-`$ref` self-contained basis — **accepted**;
2. intra-document fragment reference (`#/$defs/…`) — **accepted**;
3. external **relative** file reference — **rejected as unusable**;
4. external **absolute / `file:`** reference — **rejected as unusable**;
5. **network** reference — **rejected as unusable, without attempting retrieval**;
6. duplicate member name at depth — **rejected as unusable**;
7. `$schema` declaring a non-2020-12 dialect — **rejected as unusable**.

None of these is ever reported as `PASS` or `FAIL`.

## R3.5 Basis unusable vs payload invalid — a mandatory distinction
...
declared unusable.** r1 defines no partial-result model and r3 does not add one.
...
## R3.6 Process exit — carried from r1 unchanged

Quoted from r1, not paraphrased, because r3 changes **none** of it:
...
## R3.7 `airep.key-trust` — mechanics that implement, and never redefine, the P2-A semantic
...
## R3.8 Parity contract — extended surface (AD-14)
...
implementation of any surface above means **r3 does not pass**.
...
3. **Identifier substitution rejected.** A basis registered for one identifier is not applied to
   another.
4. **Basis mismatch is not FAIL.** A digest/bytes mismatch is a run-input condition (R3.5), never
