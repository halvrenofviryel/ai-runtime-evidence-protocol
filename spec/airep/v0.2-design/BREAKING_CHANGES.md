...
| 4 | `input.input_ref` required, input digest optional | Required input digest; the "named projection" semantics ride the wire as the optional core member `input.digest_projection` (namespaced identifier; schema-phase final-hardening decision, 2026-08-23) — absent ⇒ digest binds the full governed input, present ⇒ it binds the named projection's output | AD-06; schema design §3 | **SCHEMA** |
