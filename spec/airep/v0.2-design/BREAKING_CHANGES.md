...
| 2 | Spec pins RFC 8785, but the hash rule tolerates the reference implementation's `{record, previous}` wrapper, and shipped reference packages hash sorted-key `json.dumps` (with `ensure_ascii`/`allow_nan` variance between packages) | Exactly one rule: JCS canonical bytes, hash in place (`current` + `signature` removed; `previous` + `chain_id` retained). No wrapper. No alternatives. | AD-04 | **HASH** |
| 3 | No chain identifier; no record identifier; `previous` gives relative binding only (limit acknowledged in THREAT_MODEL.md) | `chain_id` (signed, genesis-chosen) and `record_id` required on every artifact | AD-05 | **HASH**, **SCHEMA** |
...
| 6 | `evidence[].content_hash` required only when `resolvable: false`, and only at the Verified class | `content_hash` required on every evidence entry for the authenticated class | AD-06 | **CLASS** |
...
| 9 | Class ladder named Core / Verified / Trusted; "Trusted" reachable only in opt-in strict mode | Ladder renamed (working candidate: Core / Authenticated / Witnessed); every class bound to "provenance, integrity, freshness — not truth" | AD-09 | **VOCAB** |
| 10 | `chain_witness` is the only head-witness mechanism | `chain_witness` retained for the offline case; SCITT (RFC 9943) binding profile added for transparency-service anchoring | AD-10 | ADDITIVE |
