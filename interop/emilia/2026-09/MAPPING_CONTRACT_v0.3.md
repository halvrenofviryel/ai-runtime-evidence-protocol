...
## Revision 0.3 · provisional mapping rules · 10 September 2026

**Status: PROPOSED / NOT FROZEN / NOT RUN.**

This revision records Phionyx's provisional mapping rules for the selected five-case planning scope. It is a **Phionyx-authored proposal only**: it is not approved, accepted or endorsed by anyone else, and no external party is a party to it. It does not change AIREP, EMILIA, the five-case scope, or `AIREP_EMILIA_INTEROP_CASE_SHEET_v0.2.md`.

### Fixed references

- AIREP: `v0.2.0-beta.1`, commit `8a6c01ecce457aa94330c0ed7219e4c56ebfe771`
- EMILIA public repository snapshot selected by Phionyx: [`emiliaprotocol/emilia-protocol` @ `b1275b08f91939a2330aee26a03d0a6bfda375d6`](https://github.com/emiliaprotocol/emilia-protocol/tree/b1275b08f91939a2330aee26a03d0a6bfda375d6)
- Case sheet: `AIREP_EMILIA_INTEROP_CASE_SHEET_v0.2.md`
...
The EMILIA commit above is the public source pin Phionyx selected for these mapping decisions. It is **not a frozen experiment baseline**, and no external approval of it exists or is claimed.

---

## 1. Provisional mapping decisions for the selected scope
...
2. The adapter MAY emit `executed` only when an executor-side source establishes that the exact scoped action ran under the defined action identity/projection.
...
6. Provider commitment alone MUST NOT be used as a generic substitute for an executor report unless a separately defined and independently recorded basis establishes that the selected native artifact directly establishes the AIREP action-ran predicate at the chosen action granularity. No blanket equivalence is proposed by this revision.
...
6. The mandatory `executed_action_digest` on a suppressed Execution MUST bind the exact attempted action under a defined byte/projection rule; the adapter MUST NOT copy the authorized digest merely to fill the field.
...
Phionyx inspection of the pinned public EMILIA source indicates the following, read statically from
[`packages/verify/src/aeb-consequence-conformance.ts`](https://github.com/emiliaprotocol/emilia-protocol/blob/b1275b08f91939a2330aee26a03d0a6bfda375d6/packages/verify/src/aeb-consequence-conformance.ts)
at commit `b1275b08f91939a2330aee26a03d0a6bfda375d6`:

- `truthPairValid` (line 553) admits `COMMITTED` with any effect relation.
- `terminalTruthPair` (line 560) treats a pair as terminal only for
  `PROVEN_NOT_COMMITTED` + `NOT_OBSERVED`, or `COMMITTED` with `OBSERVED_AS_REQUESTED` or `DIVERGED`.
  `COMMITTED` + `NOT_OBSERVED` and `COMMITTED` + `INDETERMINATE` are therefore **non-terminal**.
- In the `INVOCATION_OBSERVATION` branch, a non-terminal pair yields `custody: INVOKING`,
  `reconciliation: REQUIRED` and `decision: INDETERMINATE`.
- The reason-selection chain in that branch tests `TIMEOUT_AFTER_DISPATCH`, then
  `provider_outcome === 'INDETERMINATE'`, then `PROVEN_NOT_COMMITTED`, then
  `effect_relation === 'DIVERGED'`, and otherwise falls through (line 950) to
  `provider_committed_effect_observed`.

| Provider outcome | Effect relation | Terminality per `terminalTruthPair` | Reconciliation | Reason label selected by the fall-through |
|---|---|---|---|---|
| `COMMITTED` | `NOT_OBSERVED` | non-terminal | required | `provider_committed_effect_observed` |
| `COMMITTED` | `INDETERMINATE` | non-terminal | required | `provider_committed_effect_observed` |

The state and reconciliation axes remain conservative, but the `reason` label is inconsistent with
those axes for these two pairs.

**Scope of this observation.** This is a Phionyx **static source reading** of public code at the
pinned commit, plus the truth table those two functions imply. Phionyx did **not** execute the
kernel and records no runtime result; this is not a reproduced runtime behavior, and no external
party is the source of this observation.
...
The public source citations in section 2 above are the record of this observation.
...
- EP-A may support a scoped Control only after all mandatory Control fields and source bindings are defined and recorded.
...
- Retain native `DIVERGED` through a defined source reference/experiment ledger; do not invent an AIREP core `DIVERGED` enum.
...
Before any adapter run, every emitted mandatory AIREP field MUST have a defined and recorded source.
...
| `authorized_action_digest` | Bind what the control side authorized under a defined projection; never substitute `instruction_digest`. |
...
- **Adapter implementation:** an implementer who is not an AIREP first-party producer author, implementing from the pinned AIREP specification. No implementer is named or assigned here.
...
Not importing the first-party producer is necessary for the proposed independence claim but not sufficient by itself. No adapter implementer is recorded.
...
7. A recorded adapter implementer and independence disclosure.
8. Fixed toolchains, commands, signing/operator inputs, output naming and retention layout.
9. A separately defined and independently recorded input/acceptance basis for this mapping revision and its source pins.
...
- Public source pin: [`emiliaprotocol/emilia-protocol` @ `b1275b08f91939a2330aee26a03d0a6bfda375d6`](https://github.com/emiliaprotocol/emilia-protocol/tree/b1275b08f91939a2330aee26a03d0a6bfda375d6).
- AIREP target pin: `v0.2.0-beta.1`, commit `8a6c01ecce457aa94330c0ed7219e4c56ebfe771`.
- Section 2 cites the exact public source file and line numbers it was read from.
- The five v0.2 case descriptors and the original fixture remain unchanged planning inputs.
